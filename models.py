...
class HeteGNN(nn.Module):
    ...
    def forward(self, graph, inputs): # inputs:节点特征  dict {'comp': comp_feat}
...
class HyperSTGNN(nn.Module):
    def __init__(self,input_dim,output_dim,
     company_num,rel_num,
     device,com_initial_emb,g,node_features, #  node_features(节点特征字典)
     fin_seq,
     num_heads=1,dropout=0.2,norm=True,
        ...
        # self.company_emb=torch.FloatTensor(com_initial_emb.float()).to(device) # meta2path的初始向量
        ...
        self.hetegnn = HeteGNN(in_feats=input_dim, hid_feats=2*input_dim, 
             out_feats=output_dim, rel_names=g.etypes,rel_num=rel_num) # out_feats 交易类型数
    ...
    def forward(self,hete_graph,node_features,hyp_graph,idx): #  node_features(节点特征字典
        ...
        # company_basic_info=torch.zeros((self.company_num,len(company_attr[0]))) # 行业信息
        ...
        # company_emb_info=self.risk_proj(torch.cat((company_emb,risk_info),dim=1)) # 诉讼事件信息
        ...
        # g, node_features = hete_graph  #  node_features(节点特征字典)
        ...
        # company_emb_final=self.info_proj(company_emb_hyper) # 消融1 hete
        # company_emb_final=self.info_proj(company_emb_hete['company']) # 消融2 hyper

        # company_emb_and_seq_info = self.final_proj(company_emb_info) # 消融3 history seq
        company_emb_and_seq_info = self.fusion(torch.cat((self.final_proj(company_emb_info),fin_seq_info),dim=1)) # 序列信息输出与原始信息拼接并线性变换 （全量模块）
